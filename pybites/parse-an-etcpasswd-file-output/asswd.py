...
def get_users_for_shell(passwd_output: str = PASSWD_OUTPUT, grep_shell: str = DEFAULT_SHELL) -> List[str]:
    """Match the passwd_output string for users with grep_shell.
    Return a list of users.
    """
    return [line.split(":")[0] for line in passwd_output.splitlines() if line.split(":")[-1].endswith(f"/{grep_shell}")]
