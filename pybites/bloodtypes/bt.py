...
def check_bt(donor, recipient):
    ...
    """
    donor = _sanitise_input(donor)
    recipient = _sanitise_input(recipient)

    return all(comp >= 0 for comp in _particular_antigen_comp(donor, recipient))


def _sanitise_input(val: Any) -> int:
    if not isinstance(val, (int | str | Bloodtype)):
        raise TypeError

    if isinstance(val, int) and val in Bloodtype:
        return val
    elif isinstance(val, str):
        val = blood_type_text.get(val)
        if val is not None:
            return val.value
    elif isinstance(val, Bloodtype):
        return val.value
