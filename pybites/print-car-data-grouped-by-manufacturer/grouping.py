...
def group_cars_by_manufacturer(cars: list[tuple[str, str]]):
    ...
    """
    for manufacturer, models in groupby(sorted(cars, key=lambda x: x[0]), lambda x: x[0]):
        print(manufacturer.upper())
        for model in sorted(models, key=lambda x: x[1]):
            print(f"- {model[1]}")
        print("")
